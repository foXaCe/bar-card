...
`;console.info("%c BAR-CARD %c 3.2.1","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");let je=class extends oe{constructor(){super(...arguments),this._configArray=[],this._stateArray=[],this._animationState=[],this._rowAmount=1}static async getConfigElement(){return document.createElement("bar-card-editor")}static getStubConfig(){return{}}shouldUpdate(e){return be(this,e,!1)}setConfig(e){if(!e)throw new Error(Pe("common.invalid_configuration"));this._config=ve({animation:{state:"off",speed:5},color:"var(--bar-card-color, var(--primary-color))",columns:1,direction:"right",max:100,min:0,positions:{icon:"outside",indicator:"outside",name:"inside",minmax:"off",value:"inside"}},e),"horizontal"==this._config.stack&&(this._config.columns=this._config.entities.length),this._configArray=function(e){const t=[];if(e.entities){for(const i of e.entities)if("string"==typeof i){const n=ve({},e);delete n.entities;const o=ve(n,{entity:i});t.push(o)}else if("object"==typeof i){const n=ve({},e);delete n.entities;const o=ve(n,i);t.push(o)}}else t.push(e);return t}(this._config),this._rowAmount=this._configArray.length/this._config.columns}render(){return this._config&&this.hass?R`
      ...
      ${Te}
    `:R``}_createBarArray(){const e=[];for(let t=0;t<this._configArray.length;t++)(e.length+1)*this._config.columns==t&&e.push(this._config.columns),this._configArray.length==t+1&&e.push(this._configArray.length-e.length*this._config.columns);const t=[];for(let o=0;o<e.length;o++){const a=[];for(let t=0;t<e[o];t++){const e=o*this._config.columns+t,r=this._configArray[e],s=this.hass.states[r.entity];if(!s){a.push(R`
            <div class="warning" style="margin-bottom: 8px;">
              ${Pe("common.entity_not_available")}: ${r.entity}
            </div>
          `);continue}let c;if(c=r.attribute?s.attributes[r.attribute]:s.state,r.severity&&this._computeSeverityVisibility(c,e))continue;r.limit_value&&(c=Math.min(c,r.max),c=Math.max(c,r.min)),isNaN(Number(c))||(0==r.decimal?c=Number(c).toFixed(0):r.decimal&&(c=Number(c).toFixed(r.decimal)));let l=40;r.height&&(l=r.height);let d,h,p,u="stretch",g="0px 0px 0px 13px",m="right",f="row",v="left",b="height: 100%; width: 2px;";switch(r.direction){case"right":m="right",v="left";break;case"up":g="0px",m="top",f="column-reverse",v="bottom",b="height: 2px; width: 100%;"}switch(p=this._computeSeverityIcon(c,e)?this._computeSeverityIcon(c,e):r.icon?r.icon:s.attributes.icon?s.attributes.icon:pe(ce(r.entity),c),r.positions.icon){case"outside":d=R`
              <bar-card-iconbar>
            ...
            `,g="0px";break;case"off":g="0px"}const y=r.name?r.name:s.attributes.friendly_name;let _,$,w,x,S,A,k;switch(r.positions.name){case"outside":_=R`
              <bar-card-name
                class="${r.entity_row?"name-outside":""}"
                style="${"up"==r.direction?"":r.width?`width: calc(100% - ${r.width});`:""}"
                >${y}</bar-card-name
              >
            `,g="0px";break;case"inside":$=R`<bar-card-name>${y}</bar-card-name>`}switch(w=isNaN(Number(c))?"":r.unit_of_measurement?r.unit_of_measurement:s.attributes.unit_of_measurement,r.positions.minmax){case"outside":x=R`
            ...
            `;break;case"inside":S=R`
              ...
              <bar-card-divider>/</bar-card-divider>
              <bar-card-max> ${r.max}${w}</bar-card-max>
            `}switch(r.positions.value){case"outside":A=R`
            ...
            `;break;case"inside":k=R`
              <bar-card-value
                ...
                >${r.complementary?r.max-c:c} ${w}</bar-card-value
              >
            `;break;case"off":g="0px"}let C="";c>this._stateArray[e]?(C="▲","up"==r.direction?this._animationState[e]="animation-increase-vertical":this._animationState[e]="animation-increase"):c<this._stateArray[e]?(C="▼","up"==r.direction?this._animationState[e]="animation-decrease-vertical":this._animationState[e]="animation-decrease"):this._animationState[e]=this._animationState[e],isNaN(Number(c))&&(C="");const E=this._computeBarColor(c,e);let N,O;switch(r.positions.indicator){case"outside":N=R`
              <bar-card-indicator
            ...
            `}const P=this._computePercent(c,e),T=(i=this.hass,"number"==typeof(n=r.target)?n:void 0!==i&&i.states[n]?Number(i.states[n].state):NaN);if(void 0!==r.target&&isNaN(T)){a.push(R`
            <div class="warning" style="margin-bottom: 8px;">
              ${Pe("common.entity_not_available")}: ${r.target}
            </div>
          `);continue}const j=this._computePercent(T.toString(),e);let V=P,U=this._computePercent(T.toString(),e);U<V&&(V=U,U=P);let I="";r.width&&(u="center",I=`width: ${r.width}`);const z=this._animationState[e];let M="right",L=100*P,B="animationbar-horizontal";"animation-increase-vertical"!=z&&"animation-decrease-vertical"!=z||(M="bottom",B="animationbar-vertical",L=100*(100-P)),a.push(R`
          <bar-card-card
            style="flex-direction: ${f}; align-items: ${u};"
            @action=${this._handleAction}
            .config=${r}
            .actionHandler=${Se({hasHold:fe(r.hold_action),hasDoubleClick:fe(r.double_tap_action)})}
          >
            ${d} ${N} ${_}
            <bar-card-background
            ...
            >
              ...
              ${"on"==r.animation.state?R`
                    <bar-card-animationbar
                      style="animation: ${z} ${r.animation.speed}s infinite ease-out; --bar-percent: ${L}%; --bar-color: ${E}; --animation-direction: ${M};"
                      class="${B}"
                    ></bar-card-animationbar>
                  `:""}
              <bar-card-currentbar
